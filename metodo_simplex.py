...
def CrearTablaInicial(restricciones, Cantidad_restricciones, X, Y):
    ...
    columnas = 7  # Solución, x, y, s1, s2, s3, z
    ...
    for i in range(Cantidad_restricciones):
        ...
        tabla[i][i+3] = 1  # Columna de variables slack
    ...
    # Imprimir la tabla
    print("\nTabla inicial:")
    headers = ["Solución", "x", "y", "s1", "s2", "s3", "z"]  # Encabezados
...
def dibujarTablaNueva(tabla, contador):
...
def metodo_simplex(tabla, Cantidad_restricciones, X, Y):
    ...
    while NegativosEnZeta:
        ...
        valor_minimo = min(ultima_fila)  # Obtener el valor más negativo
        columna_pivote = ultima_fila.index(valor_minimo)  # Obtener el índice de ese valor
        ...
        print(f"Fila del pivote: {fila_pivote}, Ratio mínimo: {min_ratio:.3f}")
        ...
        for fila in range(Cantidad_restricciones + 1): #se suma 1 para incluir la fila z 
            ...
            for columna in range(7):# Solución, x, y, s1, s2, s3, z
                if fila != fila_pivote:
        ...
        dibujarTablaNueva(tablaAux, contadorDeTablas)
