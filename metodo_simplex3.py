...
def CreasRestricciones(Cantidad_restricciones):
    ...
    for i in range(Cantidad_restricciones):
        ...
        k = float(input(f"Ingrese el valor de k de la restriccion {i+1}\n"))
        solucion = float(input(f"Ingrese el valor de solución de la restriccion {i+1}\n"))

        restricciones[i] = [x, y, k, solucion]

    print("\nLas restricciones son las siguientes: \n", restricciones)
    return restricciones

def CrearTablaInicial(restricciones, Cantidad_restricciones, X, Y, K):
    # Aumentar el número de columnas para incluir la columna z
    
    columnas = COLUMNAS_BASE + Cantidad_restricciones  # Solución, x, y, k, s1, s2, s3, z
    ...
    for i in range(Cantidad_restricciones):
        tabla[i][0] = restricciones[i][3]  # Solución
        tabla[i][1] = restricciones[i][0]  # Coeficiente de x
        tabla[i][2] = restricciones[i][1]  # Coeficiente de y
        tabla[i][3] = restricciones[i][2]  # Coeficiente de y
        tabla[i][4 + i] = 1  # Columna de variables slack (s1, s2, ..., sn)
    ...
    tabla[Cantidad_restricciones][3] = -K  # Coeficiente de k
    ...
    # Imprimir la tabla
    print("\nTabla inicial:")
    headers = ["Solución", "x", "y", "k"]  # Encabezados
    for i in range(Cantidad_restricciones):
...
def metodo_simplex(tabla, Cantidad_restricciones, X, Y, K):
    ...
    while NegativosEnZeta:
        ...
        elif columna_pivote == columnaK:
            NuevaFilaK = fila_pivote
        ...
        for fila in range(Cantidad_restricciones + 1): #se suma 1 para incluir la fila z 
            ...
            for columna in range(COLUMNAS_BASE + Cantidad_restricciones ):# Solución, x, y, s1, s2, sn..., z
                if fila != fila_pivote:
    ...
    if NuevaFilaK != -1:
        valorDeK = tablaAux[NuevaFilaK][0]
        print(f"El valor de K esta en la fila {NuevaFilaK} columna 0 el cual es {valorDeK:.4f}\n")
    else:
        print("EL valor de K es 0")
...
def main():
    try:
        ...
        tabla = CrearTablaInicial(restricciones, Cantidad_restricciones, X, Y, K)
        metodo_simplex(tabla, Cantidad_restricciones, X, Y, K)
